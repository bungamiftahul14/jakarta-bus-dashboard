...
Original file is located at
    https://colab.research.google.com/drive/1N2NgrMd2C4faCcUvle2ZJ0uY_R1THlxl
"""
...
terminal_list = ["Semua Terminal"] + list(df['nama_terminal'].unique())
selected_terminal = st.sidebar.selectbox("Pilih Terminal", terminal_list)

if selected_terminal != "Semua Terminal":
    filtered_df = df[df['nama_terminal'] == selected_terminal]
else:
    filtered_df = df.copy()
...
with tab1:
    ...
    terminal_summary = df.groupby('nama_terminal').agg({
    ...
    fig_bar = px.bar(
        ...
        labels={'nama_terminal': 'Terminal', 'load_factor': 'Penumpang / Trip Bus'},
...
with tab2:
    ...
    monthly_trend = filtered_df.groupby(filtered_df['periode'].dt.to_period('M')).agg({
        'jumlah_penumpang_berangkat': 'sum'
    }).reset_index()
    monthly_trend['periode'] = monthly_trend['periode'].dt.to_timestamp()
...
with tab3:
    ...
    sim_df = df.groupby('nama_terminal').agg({
